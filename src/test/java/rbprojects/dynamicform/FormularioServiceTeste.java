...
public class FormularioServiceTeste {
	...
	private static final String PELO_MENOS_UM_CAMPO_DO_FORMULARIO_E_OBRIGATORIO = "Pelo menos um campo do formulario e obrigatório";
	private static final String O_VALOR_DO_TITULO_E_OBRIGATORIO = "O valor do título e obrigatório";
	...
	public void testAddTemplate() throws ValidationException {
		...
		try {
			...
			Assert.assertEquals("A descri��o do campo � obrigat�ria", e.getErrors()[0].message);
			Assert.assertEquals("Campos:1:Label", e.getErrors()[0].category);
			Assert.assertEquals("O tipo do campo e obrigat�rio", e.getErrors()[1].message);
			Assert.assertEquals("Campos:1:Tipo", e.getErrors()[1].category);
			Assert.assertEquals("O tipo do campo e obrigat�rio", e.getErrors()[2].message);
		...
		try {
			...
			Assert.assertEquals("O tipo do campo e obrigat�rio", e.getErrors()[0].message);
			Assert.assertEquals("Campos:1:Tipo", e.getErrors()[0].category);
			Assert.assertEquals("Os valores de um campo do tipo radio � obrigat�rio", e.getErrors()[1].message);
		...
		try {
			...
			Assert.assertEquals( "A descri��o do valor de um campo do tipo radio � obrigat�ria",e.getErrors()[0].message);
			Assert.assertEquals("Campos:2:Valores:1:Label", e.getErrors()[0].category);
			Assert.assertEquals("O valor de um campo tipo radio � obrigat�rio",e.getErrors()[1].message);
	...
	public void buscaFormById() throws ValidationException, BussinessException {
		try {
			...
			Assert.fail("Era esperado a exce��o:BussinessException");
		} catch (BussinessException e) {
			Assert.assertEquals("O template com o id 56a8cc1ba1971e98f0526d67 n�o foi encontrado",e.getMessage());
		}
	...
	public void removeTemplate() throws ValidationException, BussinessException{
		...
		try {
			...
			Assert.assertEquals("O template com o id 56a8cc1ca1971e98f0526d30 n�o foi encontrado",e.getMessage());
		}
		repo.save(temp);
	}
	...
	public void atualizaDados() throws ValidationException, BussinessException{
		...
		try {
			...
			Assert.assertEquals("O valor do campo Razao Social � obrigat�rio", errors[0].message);
			Assert.assertEquals("Razao Social",errors[0].category);
	
			Assert.assertEquals("O valor do campo Numero de Filhos � obrigat�rio", errors[1].message);
			Assert.assertEquals("Numero de Filhos",errors[1].category);
			
			Assert.assertEquals("O campo Telefone deve conter no m�ximo 13 caracteres", errors[2].message);
